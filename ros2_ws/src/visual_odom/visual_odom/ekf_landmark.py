...
class ExtendedKalmanFilterLandmark:
	def __init__(self, x: Coordinate, P: NDArray,  pix_coor: PixelCoordinate, R: NDArray = None ) -> None:
		...
		self.P = P
		if R is None:
			self.set_R(pix_coor)  # Initialisiere R basierend auf der Tiefe des Landmarks
		else:
			self.R = R
	...
	def sigma_R_approximation(self, kp: PixelCoordinate) -> NDArray:
		...
		z_m = kp.z/1000
		s_z = ERROR_MIN_DEPTH + ERROR_QUADRATIC_DEPTH * (z_m - (MIN_DEPTH)/1000)**2
	...
	def calculate_R_sigma_matrix(self, kp: PixelCoordinate) -> NDArray:
		...
		J_kinect = np.array([[z_m/F, 0, (kp.u-CU)/F],
						  [0, z_m/F, (kp.v-CV)/F],
						  [0, 0, 1]])
