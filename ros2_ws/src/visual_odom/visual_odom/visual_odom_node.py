...
from visual_odom.constants import (
    ...
    N_ROBOT_SAMPLES, VISUAL_ODOM_MSG_TOPIC
)
...
class VisualOdom(Node):
    ...
    def ransac(self, matches: List[cv2.DMatch], valid_kp: List[cv2.KeyPoint], valid_kp_depth: np.ndarray, valid_kp_last: List[cv2.KeyPoint], valid_kp_depth_last: np.ndarray) -> Tuple[Coordinate, float, List[cv2.KeyPoint], int]:
        ...
        # Re-compute optimal Kabsch transformation matrix parameters using the best inlier set
...
def main():
    """!
    @brief Core node runtime initialization routine executing Python profilers and tracking loops.
    """
    rclpy.init()
    node = VisualOdom()
    ...
    try:
        ...
        # Format profile statistics output
        stream = io.StringIO()
        ...
        # Save profiler file and duplicate copies to the system mounts
        profiler.dump_stats('/tmp/profile.prof')
        shutil.copy('/tmp/profile.prof', '/mnt/c/Users/lukas/Desktop/profile.prof')
        print("Profiling data successfully saved to: /mnt/c/Users/lukas/Desktop/profile.prof")
